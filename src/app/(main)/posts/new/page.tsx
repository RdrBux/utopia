	...
	return (
		<div className="main-layout">
			<main className="bg-card p-0 lg:col-start-2">
				<h1 className={`${robotoSlab.className} form-title p-6`}>Nueva publicación</h1>
				<div className="flex border-y divide-x text-gray-600">
					...
					<Link href="/posts/new?type=workout" replace={true} className={`${type === 'workout' ? 'text-gray-900 bg-gray-100' : ''} px-5 py-2.5 font-bold flex gap-2 items-center justify-center w-full hover:bg-gray-100 duration-100 hover:text-gray-900 active:bg-gray-200`}>
				...
				{type === 'general' && <NewPostForm type={'general'} />}
				{type === 'food' && <NewPostForm type={'food'} />}
				{type === 'workout' && <NewPostForm type={'workout'} />}
			</main>
		</div>
	)
}