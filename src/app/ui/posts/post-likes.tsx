import { getPostLikesCount } from "@/app/lib/data";
...
export default async function PostLikes({ postId }: { postId: string }) {
	...
	return (
		<Link href={`/posts/${postId}/likes`} className="group flex gap-1 text-sm items-center font-semibold">
			...
			<span className="group-hover:underline">{likesCount.count}</span>
		</Link>
	)
}