...
export default function UserMenu({ user }: { user: User }) {
	...
	const avatar = user.img_url && user.img_url?.length > 0 ? user.img_url : '/avatar.svg'
		...
		<div className="relative">
			...
			<div className={`${showMenu ? 'visible' : 'invisible'} ${roboto.className} z-10 absolute right-0 top-10 bg-card px-0 py-2 text-sm text-gray-700 w-40 shadow`}>
				<ul>
					...
					<li>
						<Link href={`/profile/${user.id}/settings`} className="hover:bg-gray-100 flex items-center gap-3 px-4 py-3 w-full">
							<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
